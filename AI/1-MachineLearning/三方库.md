...
矩阵乘法的规则：矩阵A的列数必须等于矩阵B的行数（也就是依次使用A矩阵的行 与 B矩阵的列 做点积，所以A矩阵行的元素个数必须跟B矩阵列的元素个数相同），才能相乘。
...
a = np.array([[3, 4, 6], [4, 7, 9], [2, 5, 8], [1, 2, 3]])
...
可以看到，两个多维数组相乘之后会得到一个新数组，新数组的行数等于A数组的行数、列数等于B数组的列数。

那么，对于两个一维数组，怎么做矩阵乘法呢？其实两个一维数组做矩阵乘法，就是点积运算，把数组各索引处的值相乘然后再相加，最后得到一个标量，而不是新的数组了（上面多维数组的矩阵乘法中，新数组中的每一个值就是一次一维数组的点积运算）。

使用numpy如何实现两个一维数组的点积运算呢，代码如下：
...
上面演示了一维数组点积的过程，在numpy中其实已经封装了，两个一维数组进行点积时，会直接给出点积结果：
