...
- 我们注意到，script标签的src、img标签的src，或者说link标签的href他们没有被同源策略所限制。jsonp就是使用同源策略这一“漏洞”，实现的跨域请求。
...
4. **类选择器**
   ...
   /* disabled状态的标签设置为灰色 */
   /* 标签中的所有属性都可以理解为 类选择器。因为其实 class=xxx 也是标签的一个属性而已 */
   [disabled]{
     color: grey
   }
   /* 伪类选择器和类选择器同级 */
...
<img src="./images/frontend_020.png" style="float: left;">
...
<img src="./images/frontend_021.png" style="float: left;">
...
**页面在浏览器中打开，img标签的src属性会自动请求其图片地址。**

**实现懒加载先使用自定义属性(例:data-src)来保存图片地址，避免img的自动加载。然后js监听父元素的scroll事件，当此图片出现在可视区域后，js再把自定义src设置为图片src，实现懒加载。**
...
4. **用伪元素的方式给父元素后面添加一个块元素并设置清除属性 clear:both**
   ...
   <head>
       ...
       <style>
           ...
           .clearfix:after {
               ...
               display: block; # 内墙法要求元素是块级，因为 行内元素会忽略掉 margin 等属性。
...
​		**三角形往哪个方向，那个方向无需设置border，而相反方向设置border颜色，相邻两边的border设为透明。**这样就可实现各个方向的三角形。实心三角形利用CSS中的伪元素（：before）实现，再利用border的transparent属性即可达到效果。而空心三角形是在空心三角形的基础上再加上伪元素（：after）实现。伪元素（：before）实现的是一个实心的三角形，伪元素（：after）实现的是空心的三角形，进而把实心的三角形覆盖，利用绝对定位的top与left的差值决定了三角形线的粗细而达到效果。
...
>
> 所谓 引用，就是 对象 保存在内存中的地址。我们的变量就是指向这个内存地址的，当我们使用这个变量时，就是通过 变量 找到对象在内存中暴露的地址，然后再通过内存地址取出 对象 做运算。
>
...
**indexOf: **从数组的开头（位置 0）开始向后查找。返回要查找的值第一次在数组中出现的位置索引，没找到返回-1。
...
默认情况下sort()方法没有传比较函数的话，默认按字母升序，如果元素不是字符串的话，会调用toString()方法将元素转化为字符串的Unicode(万国码)位点，然后再比较字符。所以用默认方法排序 数值 是有问题的。
...
> DOM（文档对象模型）是HTML和XML文档的编程接口。它提供了对文档的结构化的表述，并定义一种方式可以从程序中对该结构进行访问，从而改变文档的结构、样式和内容。DOM将文档解析为一个由节点和对象（包含属性和方法的对象）组成的结构集合。
...
> var b = {m: 20}; // 变量b 引用一个对象。 b 在栈内存中保存一个 堆内存地址，通过这个地址就能找到 堆内存中的对象
> ```
>
> 当我们执行 b={m:30}时，堆内存就有新的对象{m：30}，栈内存的b指向新的空间地址( 指向{m：30} )，而堆内存中原来的{m：20}就会被程序引擎当作垃圾回收掉，节约内存空间。
...
// 当程序执行完var inner = outer()，其实outer的执行环境并没有被销毁，因为他里面的变量 name 仍然被被inner的函数作用域链所引用，当程序执行完inner(), 这时候，inner和outer的执行环境才会被销毁调；《JavaScript高级编程》书中建议：由于闭包会携带包含它的函数的作用域，因为会比其他函数占用更多内存，过度使用闭包，会导致内存占用过多。
```
...
function  showId() {
  ...
  el.onclick = function(){
    alert(el.id)   // 这样会导致闭包引用外层的el，当执行完 showId 后，el 仍然无法释放
  }
}
...
<script>
    ...
    parent.addEventListener("click", call_1); // 第三个参数默认是 false，即 默认监听事件冒泡
...
1. JSON.parse(JSON.stringify())	原理是现将对象转成一个新的字符串返回，而原对象不受影响；再将字符串解析成一个新的对象。
...
深拷贝会另外创造一个一模一样的对象，新对象跟原对象不共享内存，修改新对象不会影响原对象。
...
call 方法第一个参数是要绑定给this的值，后面传入的是多个单列参数。当第一个参数为null、undefined的时候，默认指向window。
...
fn1()
fn1.call(null); // 无任何参数调用时，等价于 call 的时候，传入 null，根据浏览器规则，传入null时，就将默认的上下文 window 传入，所以此时 this 是指向 window的。
...
- bind参数的使用
  ...
  bind 方法在被调用时，还可以预先根据参数位置，传入一些已知的位置参数。类似于python中的偏函数。
...
- 当鼠标移动到元素上时就会触发 mouseover 事件。



**区别：** 

- mouseover 鼠标经过自身盒子会触发该事件，当经过了子盒时也会会触发，和冒泡原理差不多；但 mouseenter 只会经过自身盒子才会触发。

**原因：**mouseenter 不会冒泡。和 mouseenter 搭配鼠标离开事件 mouseleave，同样不会有冒泡行为。
...
监听属性 watch 是监听 data() 函数中已经定义的属性。监听函数需要与被监听的属性同名。
...
在 Vue 中，插槽（Slots）是父组件将内容“注入”到子组件的另一种方式。是使用模板代码来完成的。在子组建中使用`<slot></slot>`标签，中间添加默认值，就相当于在子组件中申请了一块空间，当父组件向子组件中添加内容时，子组件就将其添加到 slot 预留的位置上。
...
router，可以理解为一个容器，或者说一种机制，它管理了一组route。

简单来说，route只是进行了URL和组件的映射，而在当接收到一个URL请求之后，去路由映射表中根据URL查找到相应的组件，并将组件添加到 App.vue 的`<router-view>`占位标签中。 这个过程是由router来处理的。



### 路由配置使用步骤

路由， 控制组件之间的跳转，不用实现请求、不用页面刷新，直接跳转。

#### 1. 引入 vue-router

> 安装 vue-router 的方式：
>
> 1. 在创建 vue 项目时，勾选 router 组件即可；
>
> 2. 在项目内部安装：
>
>    ```vue
...
```js
// 在 src/router/index.js 中引入
import Vue from 'vue'
import VueRouter from 'vue-router'

Vue.use(VueRouter) // 注册 router
```
...
Vue.use(VueRouter) // 告诉 vue 使用 router

// 配置多个路由
const routes = [
  {
    path: '/chat_room',  // 显示在浏览器地址栏的 url 路径
    name: 'ChatRoom',  // 路由的名字
    component: () => import('../views/chatroom.vue'),  // 路由映射的组件
  },
  {
    path: '/cars',
    name: 'Cars',
    component: () => import('../views/cars.vue'),
    meta: {  // 为路由配置一些元属性，比如下面的 title 即是网页的title；requireAuth 表示权限认证
      title: "汽车之家-新闻",
      requireAuth: true
    }
  },
    ...
    children: [ // 子路由。主路由映射的组件中，需要用 <router-view> 占位标签 来接收子路由映射的组件。
      {
        path: 'detail', //  以“/”开头的嵌套路径会被当作根路径，所以子路由上不用加“/”;在生成路由时，主路由上的path会被自动添加到子路由之前，所以子路由上的path不用在重新声明主路由上的path了。
        name: 'detail',
        component: () => import('../views/userInfo.vue'),  //  填充到 主路由 组件的 <router-view> 标签中。
      }
    ]
  }
]
...
主要是用 router 的 push、replace 方法实现路由跳转。

```js
// 1. 不带参数
this.$router.push('/home')  // 不带参时，可以直接传入 path。
...
- **带参跳转**
  ...
  - 通过属性 params、query 传参
    ...
    **注意事项：**
    ...
    1. 使用 query 传参，会在浏览器的url栏看到传的参数类似于get请求。使用 params 传参则不会，类似于post请求；
    2. 如果是用 path，params 将会被忽略（即如果要使用params传参，则一定要使用name），但 query 不关心使用name还是path；
    3. **使用 query 传参，刷新页面时不会造成路由传参的参数丢失。params 传参，在页面刷新后，参数会丢失；**
    4. router.push 和router.replace 的区别是：replace不会向 history 添加新记录，而是替换掉当前的 history 记录，即使用replace跳转到的网页‘后退’按钮不能点击。
    5. router.go() 方法不再表示跳转，而是接受一个整型参数，作用是在 history 记录中 向前（正数）或 后退（负数）多少步，刷新 就传入 0。

  - URL 路径传参

    路径传参，对应的路由配置区别于上面的传参方式，需要定义变量接收路径上的参数。而且 **刷新页面参数不会丢失**。

    ```js
    // 路径传参
    {
         path: '/particulars/:id',  // 在path中添加/:id来对应 $router.push 中path携带的参数。
         name: 'particulars',
...
<template>
  <div>
    <!-- router-link标签和 a 标签类似，这里的属性 to 指向目标 url。切记，这里只能是目标 path，不能使用 name -->
    <router-link to="/chat_room">聊天室</router-link>
  </div>
</template>
```
...
动态路由详解：https://www.jianshu.com/p/4f2566b67989
...
router 是 VueRouter 的一个对象，通过 Vue.use(VueRouter) 和 VueRouter 构造函数得到一个router的实例对象，这个对象中是一个全局的对象，他包含了所有的路由和许多关键的对象、属性。

**举例：history 对象**

```js
$router.push({path:'/home'}); // 本质是向 history 栈中添加一个路由，在我们看来是 切换路由，但本质是在添加一个history记录
...
route 是一个路由的实例对象，每一个路由都会有一个 route 对象，它是一个局部的对象，在路由映射的组件中可以用这个 route 对象。通过调用相应的属性，可以获取路由对应的 name、path、params、query 等。
...
Vue实例有一个完整的生命周期，也就是从开始 **创建实例、初始化数据、编译模板、挂载DOM、渲染 =>更新=>渲染、销毁** 等一系列过程，我们称这是Vue的生命周期。通俗说就是Vue实例从创建到销毁的过程，就是生命周期。
...
<img src="./images/frontend_011.jpg" style="width: 60%; float: left">

1. `new Vue()` 创建 vm 实例；

2. `initLifecycle(vm)、initEvents(vm)、initRender(vm)`：在创建的vm实例上初始化生命周期、事件、渲染相关的属性；

3. 执行 **beforeCreate** 函数，此时组件的选项对象还未创建，el 和 data 并未初始化，因此无法访问methods， data， computed等上的方法和数据；

4. `initInjections(vm)、initState(vm)、initProvide(vm)`：初始化数据：inject、state、provide。initState 的作用是初始化 props、data、methods、watch、computed 等属性；

5. 执行 **created** 函数，此时 vm实例 已完成以下配置：数据观测、属性和方法的运算，watch/event事件回调，完成了data 数据的初始化，但挂载阶段还没有开始，vm.$el 属性目前不可见。 此时可以调用methods中的方法，改变data中的数据（不会触发 updated 钩子），并且修改可以通过vue的响应式绑定体现在页面上，获取computed中的计算属性等等；

   > 通常我们可以在这里对实例进行 **预处理**，也可以在这里发ajax请求，值得注意的是，这个周期中是没有什么方法来对实例化过程进行拦截的，因此如果有某些数据必须获取才允许进入页面的话，并不适合在这个方法发请求，建议在组件路由钩子beforeRouteEnter中完成。

6. `vm.$mount(vm.$options.el)`: 编译模板，把data里面的数据和模板生成html，完成了 el 和 data 初始化，注意此时还没有挂在html到页面上；

7. 执行 **beforeMount** 函数，挂载开始之前被调用；

8. `vm._render()` => `vm._update()` => `vm.__patch__()`：先执行`vm._render`方法，即调用createElement生成虚拟DOM，即 虚拟结点 VNode ，每个VNode有children ，children 每个元素也是⼀个 VNode，这样就形成了⼀个 VNode Tree；再调用`vm._update`方法进行首次渲染，vm._update方法核心是调用 vm.patch 方法，根据生成的VNode Tree 递归调用 createElement 方法创建真实DOM Tree挂载到DOM上；

9. 执行 **Mounted** 函数，此时已完成挂载，模板中的HTML渲染到HTML页面中，这里已经可以进行 DOM 操作；

10. `data changes`：数据更新，nextTick中执行`flushSchedulerQueue`方法，该方法会执行watcher队列中的watcher。执行watcher时会执行watcher的before方法，即调用beforeUpdate生命周期钩子函数；

11. 执行 **beforeUpdate** 函数；

12. `Virtual DOM re-render and patch`：重新render生成新的Virtual DOM，并且patch到DOM上；利用diff算法进行对比之后重新渲染；

13. 执行**updated** 函数，数据已经更改完成，DOM也重新render完成；

14. `vm.$destroy()`：启动卸销毁过程；

15. 执行**beforeDestroy** 函数：调用beforeDestroy生命周期钩子函数；

16. `Teardown watchers, childcomponents and event listeners`：执行一系列销毁动作，在 $destroy 的执行过程中，它又会执行`vm.__patch__(vm._vnode, null)` 触发它子组件的销毁钩子函数，这样一层层的递归调用，所以 destroyed 钩子函数执行顺序是先子后父，和 mounted 过程一样。

17. 执行**destroyed** 函数：调用destroyed 生命周期钩子函数。
...
响应式编程，即为保持 模型 和 视图 的同步，理解为 后台数据一旦更新，可以 快速的响应到视图界面上。

这也带来一个问题，就是 模型 和 视图 要保持数据同步，必然会产生耦合，耦合度越高，给后期维护带来的压力就越大。

为了解决上述问题就发展出了各种 `MV*` 框架模式（`MV*`理论本身是一种模式，但它的各种实现又叫 `MV*` 框架），它是通过解决M和V的耦合问题来实现界面分离。
...
<img src="./images/frontend_012.png" style="float: left; width: 30%">
...
- 桌面UI MVC
  ...
  经典实现有[PureMVC](http://puremvc.org/)，PureMVC 是在基于MVC模式建立的一个轻量级的应用框架。它的Model与View的交互是通过观察者模式实现的，当Model有变化要通知View的时候Model把消息发送给通知分发中心，然后通知分发中心把消息发送到View。

  

**总结：**

MVC 实现了视图和模型的分离，避免了视图和模型糅合在一起，当视图改变的时候只要业务逻辑没变就不需要改变模型；但是它有一个缺点缺点是因为MVC中的控制器并不能直接更新视图，所以MVC并不能实现视图和模型的完全分离，视图依然依赖模型的数据（数据结构）来显示，也就是说视图依赖模型。
...
<img src="./images/frontend_013.png" style="float: left; width: 30%">
...
<img src="./images/frontend_014.png" style="float: left; width: 30%">
...
MVVM 用 视图模型 代替了MVP中的展示器，视图模型和视图实现了双向绑定，当视图发生变化的时候视图模型也会发生改变，当视图模型变化的时候视图也随之变化。
...
**数据劫持:** vue.js 则是采用 **数据劫持** 结合 **发布者-订阅者模式** 的方式，通过`Object.defineProperty()`来劫持各个属性的`setter`和`getter`，在数据变动时发布消息给订阅者，触发相应的监听回调。
...
vue是通过数据劫持的方式来做数据绑定的，其中最核心的方法便是通过`Object.defineProperty()`来实现对属性的劫持，达到监听数据变动的目的。要实现mvvm的双向绑定，就必须要实现以下几点：

1. 实现一个数据监听器Observer，能够对数据对象的所有属性进行监听，如有变动可拿到 最新值 并通知 订阅者
...
实现代码：https://github.com/zhangjian-ai/Practice/tree/master/MVVM
...
父组件向子组件传递数据：**父组件通过为子组件标签添加属性传递数据；子组件通过`props`属性接收数据**

注意：**props 中的属性值，子组件是不能修改的。想要使用该值，可以将其赋值给data中的变量**
...
- **示例一：简单示例**
  ...
  <template>
    <div>
      <!-- 通过属性绑定传递name 通过直接赋值传递age -->
      <sub_demo :name="name" age=18></sub_demo>
    </div>
  </template>
  <script>
  import sub_demo from "@/views/sub_demo.vue";
  ...
  <img src="./images/frontend_015.png" style="float: left; width: 50%">

- **示例二：prop 验证**

  我们可以为组件的 prop 指定验证要求。主要的验证属性有`type`、`required`、`default`、`validator`。
  ...
  export default {
    props: {
      // 验证 name 是否为 String 类型
      name: String,
      // 验证 age 是否为 Number 类型，且 是一个合法的值
      age: {
          type: Number,
          validator: function(value) {  // 自定义校验规则
              if (0 < value && value < 136) return true;
              return false;
          }
      },
      // 验证 contry 是否有值，没有就使用默认值
      contry: {
          type: String,
          default: "China"
      },
  ...
  <template>
    <div>
      <!-- 通过属性绑定传递name 通过直接赋值传递age -->
      <sub_demo :name="name" :age=age sex="男"></sub_demo>
    </div>
  </template>
  <script>
  import sub_demo from "@/views/sub_demo.vue";
  ...
  执行结果：此时控制台是有 Invalid prop 告警的。

  <img src="./images/frontend_016.png" style="float: left; width: 50%">



### 子组件向父组件传递

子组件向父组件传递数据：**子组件通过`$emit()`触发一个事件；父组件监听事件并执行回调**
...
- **示例一：**
  ...
  <template>
    <div>
      <!-- 父组件监听触发的事件并执行回调 -->
      <sub_demo @changeName="newValue => name = newValue" :initName="name"></sub_demo>
      <p>+++++++++我是父组件++++++++++</p>
      <p>子组件的名字是：{{ name }}</p>
    </div>
  </template>
  ...
  执行结果：变更子组件的值，父组件的值也跟随变化

  <img src="./images/frontend_017.png" style="float: left; width: 50%">
...
**下面介绍的是 Vue 2.x 中过滤器的使用方法。**

Vue.js 允许在表达式后面添加可选的过滤器，以管道符表示。

过滤器的本质是一个函数，接受管道符前面的值作为初始值（即 函数的第一个入参），同时也能接受额外的参数，返回值为经过处理后的输出值。多个过滤器也可以进行串联。
...
<!-- 串联过滤器。filterB 接收 filterA 的结果作为第一个入参 -->
...
局部过滤器，在组件内部定义在属性`filters`中，仅在当前组件有效。
...
注意：如果 存在同名的全局和局部过滤器，调用时 采取就近原则，即 优先使用 局部过滤器。
...
在本节中，将会使用到`Vue.extend`方法，先做一个简单介绍。

> 在使用`element-ui`的过程中，在使用到类似`this.$message`功能的时候，会觉得这个功能真的非常方便，不用`import`入组件，全局都可以调用。它就是通过`Vue.extend` + `$mount`实现。

`Vue.extend`返回的是一个 **扩展实例构造器**，也就是一个预设了部分选项的 Vue 实例构造器。其实它就像构造函数，构造函数中会事先定义好一些属性，`new`出来的对象也就默认有构造函数中的属性，同理`Vue.extend`也是如此。



### 理解插件

 `Vue 插件` 和 `Vue组件` 只是在 `Vue.js` 中包装的两个概念而已，不管是插件还是组件，最终目的都是为了实现逻辑复用。它们的本质都是对代码逻辑的封装，只是封装方式不同而已。在必要时，组件也可以封装成插件，插件也可以改写成组件。
...
这里开发一个 toast 插件，下面的示例仅体现 插件 的开发思路逻辑，要实际使用还需要适配优化。

一般在项目开发过程中，会新建一个`plugins`文件夹放编写的插件，一个插件对应一个文件夹，文件夹里包含两个文件，一个js文件和vue文件。
...
// 构建一个 toast 扩展示例构造器。.vue 文件导入后是不能直接实例化的，所以需要用 extend 方法将其转变成一个可以实例化的构造器。
const ToastConstructor = Vue.extend(Toast)

// 定义一个函数
const toast = function(options = {}) {
    // 创建 Toast 实例，通过构造函数传参
    // 调用 $mount() 方法，将当前实例 渲染成真实 DOM
    const toastInstance = new ToastConstructor({
        data: options
    }).$mount();

    // 挂载 DOM 到 html 的 app 根元素上
    let app = document.getElementById("app");
    app.appendChild(toastInstance.$el);

    // 添加定时器，删除 DOM
    let timer = setTimeout(() => {
        clearTimeout(timer);
        app.removeChild(toastInstance.$el);
    }, 1000)

}

// 导出对象 添加 install 属性，必须是一个函数
export default {
    install: Vue => {
...
效果图：点击按钮展示 toast ，1s 后消失

<img src="./images/frontend_022.png" style="width: 66%; float: left;">
...
当组件 `<style>` 标签有 `scoped` 属性时，在样式被真正使用之前，通过 PostCSS 为 样式 和 标签 添加一个相同的随机属性，这就使得组件的 CSS 只作用于当前组件中的元素。
...
- 父组件去掉`scoped`作用域，那样作用域就是全局，还会影响其他任何组件。不推荐。
- 有作用域
  - 默认只能影响根节点
    - F12审查元素，找到子组件根节点类名，在父组件为该类设置样式
    - 在父组件中给 子组件 添加一个 class，它会自动添加到子组件根节点的 class 中
  - 如果需要影响的更深（修改子组件内部样式），则使用深度选择器：`>>>`、`/deep/`、`::v-deep`
