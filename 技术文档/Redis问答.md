...
Redis 主从备份核心是通过 `replicaof`（或配置文件）将从节点指向主节点，实现全量 + 增量数据同步，可通过 `info replication` 查看角色 / 连接状态，或测试主写从读是否一致。主从复制的过程分为三个阶段：

1. **同步阶段**：从节点第一次连接主节点时，会触发全量同步，主节点生成 RDB 快照并发送给从节点，从节点加载快照完成数据初始化。
2. **命令传播阶段**：全量同步完成后，主节点会把后续的写命令（如 set、hmset 等）实时发送给从节点，从节点执行这些命令，保证数据和主节点一致。
3. **重连恢复**：如果主从网络中断，重连后会触发增量同步，只同步中断期间的增量数据，无需全量同步（Redis 2.8+ 支持）。
