...
> 实现从csv文件中读取测试数据。
>
> - Filename: 指保存信息的文件目录，可以是相对或者绝对路径。
...
>   ```
>
> - **勾选“interpret condition as variable expression”**	这时expression中不能直接填写条件表达式，需要借助函数将条件表达式计算为true/false，可以借助的函数有`__jexl3`和`__groovy`。
>
>   ``` java
...
​		正则表达式的示例一，我们单模版取多个值时，取值时按 image_1，image_2这样取值。然后在foreach中输入前缀，然后输入起止索引，再勾选上 _ 间隔，就可以对image循环取值。
...
- ${list_1_g1}获取第一模板的第一个数据， ${list_2_g1}获取第一模板的第二个数据
- ${list_1_g2}获取第二模板的第一个数据， ${list_2_g2}获取第二模板的第二个数据
...
> 根据相应结果判断请求是否成功，可根据 响应文本、响应头 等来做断言。
...
- 在监听器中添加插件，为当前插件配置要监听服务器哪些性能指标，IP 为服务器公网IP，PORT 为启动监听服务时的端口号。
...
- master 配置
  ...
  - jmeter.properties文件中 server.rmi.ssl.disable 属性改为true，不使用加密认证传输
...
- 特别注意：
  ...
  - master机器上的执行脚本有参数配置文件，比如 csv等，则应该在slave机器对应的目录上都copy放置一份
...
> - 异常测试：又称为失败测试。比如在负载均衡的架构中，测试节点宕机、节点掉线等。
