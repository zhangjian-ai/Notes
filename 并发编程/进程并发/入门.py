...
'''
打印结果说明：
    52611 19029 1  # py文件开始执行，打印一次'1'。当前进程为运行中的py文件；父进程为pycharm
    __main__
    52611 19029 2  # 执行到判断条件if __name__ == '__main__': ，条件为真，执行if下面的代码，打印一次'2'，当前进程任然是py文件
    ...
    52613 52611 3  # 继续执行 func函数，现在函数被调用，打印一次'3'
    zhangjian      # 继续打印一次参数
    __mp_main__    # 当前运行的不是'__main__'了，所以后续不再执行
'''
